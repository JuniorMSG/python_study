...
def covid19_01():
    ...
    data_status = df.groupby('date')['total_cases', 'total_deaths', 'new_cases', 'new_deaths'].sum()
    data_status.sort_index()

    ta = plt.figure(figsize=(10, 10), tight_layout=True, num='covid19')
    ax = sns.lineplot(data=data_status, linestyle='--')
    ax.set_yscale('log')
    ...
    m = folium.Map(location=[37.566381,	126.977717])
    m.save("image/02_EX05_covid19_data/index.html")

    m = folium.Map(location=[37.566381, 126.977717], zoom_start=10)
    m.save("image/02_EX05_covid19_data/index2.html")

    m = folium.Map(location=[37.566381, 126.977717], zoom_start=18)
    folium.Circle([37.566381, 126.977717], radius=100).add_to(m)
    m.save("image/02_EX05_covid19_data/index3.html")

    m = folium.Map(location=[37.566381, 126.977717], zoom_start=18)
    folium.Circle([37.566381, 126.977717], radius=100, color='red', fill=True, fill_color='red', popup='hi ~').add_to(m)
    m.save("image/02_EX05_covid19_data/index4.html")
    ...
    df_coordinate = pd.read_csv("data_file/countries_codes_and_coordinates.csv", engine='python', error_bad_lines='false')
    import re

    # 데이터 변경
    df_coordinate['Alpha-3'] = df_coordinate['Alpha-3 code'].apply(lambda x: re.sub('[^a-zA-Z]', ' ', x).strip())
    df_coordinate['Latitude (average)']     = df_coordinate['Latitude (average)'].apply(lambda x: re.sub('[^0-9.-]', ' ', x).strip()).astype(float)
    df_coordinate['Longitude (average)']    = df_coordinate['Longitude (average)'].apply(lambda x: re.sub('[^0-9.-]', ' ', x).strip()).astype(float)
    # df_coordinate['Latitude (average)']
    # df_coordinate['Longitude (average)']

    df_covid19_world_data = df.groupby(df['iso_code'])['total_cases', 'total_deaths', 'new_cases', 'new_deaths'].sum()
    df_covid19_world_data = df_covid19_world_data.reset_index()


    df_covid19 = pd.merge(df_covid19_world_data, df_coordinate[['Alpha-3', 'Latitude (average)', 'Longitude (average)']], left_on='iso_code', right_on='Alpha-3', how='inner')

    m = folium.Map(zoom_start=30)

    for n in df_covid19.index:
        folium.Circle([df_covid19['Latitude (average)'][n], df_covid19['Longitude (average)'][n]], radius=df_covid19['total_cases'][n]/5000, color='red', fill=True, fill_color='red', popup='hi ~').add_to(m)

    m.save("image/02_EX05_covid19_data/index5.html")
